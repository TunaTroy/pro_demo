  ...
  for (let year = minYear; year <= maxYear; year++) {
    aChartData.push({
      ...
      Count: yearCountMap[year] || 0,
       ...
       _loadTopRequesterTable: function (sPeriodKey) {
  BusyIndicator.show(0);
  ...
  this.oOData.read("/PRsSet", {
    ...
    success: (oData) => {
      ...
      const oRange = this._getDateRange(sPeriodKey);
      const aFiltered = aResults.filter((r) => 
        r.Badat instanceof Date &&
        r.Badat >= oRange.start &&
        r.Badat <= oRange.end
      );
