...
], function (Controller, JSONModel, Filter, FilterOperator, MessageBox, MessageToast, DateFormat, Spreadsheet, Fragment) {
  "use strict";
  ...
  return Controller.extend("demodashboard.controller.RFQList", {
    formatter: {
      formatDate: function (sDate) {
        if (!sDate) return "";
        const oDate = new Date(parseInt(sDate.replace(/\/Date\((\d+)\)\//, "$1")));
    ...
    onInit: function () {
      ...
      // Load RFQ List (chỉ Bstyp = 'A')
      this._loadRFQList();

      // Attach route matched
      const oRouter = this.getOwnerComponent().getRouter();
      oRouter.getRoute("RFQList").attachPatternMatched(this._onRouteMatched, this);
    },
    ...
    _loadRFQList: function () {
      ...
      this.oODataModel.read("/ProcurementHeaderSet", {
        ...
        success: (oData) => {
          ...
          // Tính tổng giá trị + đếm item
          aRFQs.forEach(rfq => {
            rfq.TotalValue = 0;
            rfq.ItemCount = rfq.ToItems?.results?.length || 0;
            (rfq.ToItems?.results || []).forEach(item => {
              const netwr = parseFloat(item.Netwr) || 0;
              rfq.TotalValue += netwr;
            });
            rfq.TotalValue = rfq.TotalValue.toFixed(2);
            // Gắn tên vendor
            rfq.VendorName = this._getVendorName(rfq.Lifnr);
          });
          this.getView().setModel(new JSONModel(aRFQs), "rfq");
          this.oViewModel.setProperty("/busy", false);
        },
        error: (err) => {
          MessageBox.error("Error loading RFQ List");
          this.oViewModel.setProperty("/busy", false);
        }
      });
    },
    ...
    onFilterSearch: function () {
      ...
      // RFQ Number
      const aEbeln = this.byId("ebelnFilter").getTokens().map(t => t.getKey());
      ...
      this.byId("rfqTable").getBinding("items").filter(aFilters.length ? new Filter(aFilters, true) : []);
    },

    onValueHelpEbeln: function () {
      // Tái sử dụng ValueHelpDialog (có thể tạo fragment riêng)
      MessageToast.show("Value Help for RFQ Number - coming soon!");
    },

    onValueHelpVendor: function () {
      MessageToast.show("Value Help for Vendor - coming soon!");
    },
    ...
    onExportExcel: function () {
      ...
      const oSettings = {
        ...
        dataSource: this.getView().getModel("rfq").getData(),
    ...
    onOpenNotes: function () {
      MessageToast.show("Notes panel - coming soon! (giống PR List)");
    },
